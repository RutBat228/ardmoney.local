...
if ($usr['rang'] != "Мастер участка") {
    if ($usr['admin_view'] == 1) {
        ...
        if (isset($_GET['current_user'])) {
            ...
            $sql .= $view_complete . " AND visible = 1 ORDER BY `date` DESC, `id` DESC";
        }
    }
} else {
...
while ($row = mysqli_fetch_array($res_data)) {
    ...
    if ($data_tmp === $date_current) {
        ...
        if ($usr['admin'] == 1 || in_array($usr['fio'], [$row['technik1'], $row['technik2'], $row['technik3'], $row['technik4'], $row['technik5'], $row['technik6'], $row['technik7'], $row['technik8']])) {
            echo "<a href='?delete=" . $encodedStr . "' data-barba-prevent='self' style='color: white;text-decoration: none;'><span class='badge bg-danger rounded-pill'>X</span></a>";
        }
